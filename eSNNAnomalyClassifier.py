...
class eSNNAnomalyClassifier(AnomalyModel):
    def __init__(self, shape=1, ni_size=10, no_max_size=50,
        ...
        self.mod = mod
        self.C = C
        self.errror_working_policy = errror_working_policy
    ...
    def __before_prediction_start(self, X):
        ...
        self.errors = np.abs((predicted_values - current_values)).T.tolist()
    ...
    def __update_neuron(self, index):
        ...
        n_s.weight = (n_s.weight * n_s.M + self.candidate_output_neuron.weight) / (n_s.M + 1)
    ...
    def predict_anomaly_proba(self, point)->float:
        ...
        if len(self.x_values_list) >= self.min_size + self.dimension:
            ...
            self.errors.append((np.abs(curr_y.squeeze() - X[-1])).T.tolist())
